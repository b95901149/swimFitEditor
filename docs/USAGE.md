...
以下以專案內附的示範檔 `fixtures/23505296923_ACTIVITY.fit`（2026-07-06、25 m 泳池、約 60 分鐘）為例。手錶在轉身時把一趟拆成兩段，圖表上會出現**明顯矮於鄰近長條**、且帶**橘色邊框**的短趟。

#### 合併前

載入後統計：**76 趟、1900 m**（尚未修正）。

![實例 — 載入後合併前（76 趟 / 1900 m）](images/07-example-before-merge.png)

放大檢視第 14 趟附近，可看到多根遠低於 30 秒門檻的短長條（橘框）夾在正常趟之間：

![實例 — 短趟特寫（合併前）](images/08-example-short-bars-before.png)
...
保持門檻 **30 秒**，按 **⚡ 自動合併短趟**。本例共合併 **10 段**短趟入相鄰同泳姿趟次。

#### 合併後

統計卡會顯示修正後數值，並標註原始趟數與距離：

![實例 — 自動合併後（66 趟 / 1650 m）](images/10-example-after-merge.png)

| 項目 | 合併前 | 合併後 | 變化 |
|------|--------|--------|------|
| 有效趟數 | 76 | **66** | −10 趟 |
| 總距離 | 1900 m | **1650 m** | −250 m |
| 平均配速 | 3:11/100m | 3:40/100m | 更接近真實單趟節奏 |

同一區段合併後，短長條消失，相鄰趟的時間回到合理範圍：

![實例 — 短趟特寫（合併後）](images/11-example-short-bars-after.png)

日誌區會列出每一筆合併紀錄（例如「第 14 段 15.9s 併入後段 → 51.2s」）：

![實例 — 自動合併日誌](images/12-example-merge-log.png)

> **說明：** 總游動時間（60:37）與划手數在合併後**不變**；減少的是多算出來的「幽靈趟」，因此總距離與趟數下降、配速變慢，才會更接近你實際的單趟速度。
