...
Garmin 手錶常在**轉身**時多記一趟：同一趟 25 m 被拆成「正常長條 + 一根很矮的短長條」。本工具的核心功能 **⚡ 自動合併短趟** 會把這些短趟併回相鄰的同泳姿趟次，**一鍵**修正趟數與距離。若手錶泳姿標記混亂，可再按 **🏊 全部改此泳姿**（以下範例統一為**蛙式**，長條全變藍色）。

**操作：** 載入 `.fit` → **⚡ 自動合併短趟** → 選 **蛙式** → **🏊 全部改此泳姿** → 下載 `_fixed.fit`
...
| 合併前 — 多根橘框短長條 | 合併後 — 短趟消失，並 **🏊 全部改為蛙式**（全藍長條） |
|:---:|:---:|
...
全貌條狀圖（合併前 76 根 → 合併後 66 根，泳姿已統一為蛙式）：

| 合併前 | 合併後（自動合併＋全改蛙式） |
|:---:|:---:|
| ![合併前全貌](images/07-example-before-merge.png) | ![合併後全貌](images/10-example-after-merge.png) |

工具列與修正後圖表（可見 **蛙式** 下拉選單與 **🏊 全部改此泳姿**）：

![修正結果 — 自動合併後全改蛙式](images/13-example-stroke-all-result.png)
...
保持門檻 **30 秒**，依序：**⚡ 自動合併短趟** → 選 **蛙式** → **🏊 全部改此泳姿**。日誌會記錄合併明細與「一鍵：全部 66 段改為 蛙式」：

![實例 — 自動合併與統一泳姿日誌](images/12-example-merge-log.png)
