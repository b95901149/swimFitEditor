...
Garmin 游泳 `.fit` 檔的分趟（length）修正工具。手錶常在**轉身**時把同一趟 25 m 誤判成兩筆記錄，
導致趟數與距離被多算、單趟配速失真。本工具讓你把被拆開的趟**合併回去**，重新計算速度與配速，
並匯出可重新匯入 Garmin Connect / Strava 的修正版 `.fit`。

靈感來自 swimdata.org，但**完全在本機瀏覽器執行，檔案不會上傳到任何伺服器**。

## 主要特色：⚡ 自動合併短趟 ＋ 🏊 統一泳姿

**一鍵修正誤拆趟** — 載入 `.fit` 後按 **⚡ 自動合併短趟**，工具會找出低於門檻（預設 30 秒）
的異常短趟（長條圖上**很矮、橘色邊框**），自動併入相鄰的同泳姿趟次。總游動時間與划手數不變，
修正的是多算出來的幽靈趟與距離。

若泳姿顏色混雜，可再選 **蛙式** 並按 **🏊 全部改此泳姿**，整段長條圖會統一為同一顏色（範例為藍色蛙式）。

**建議流程：** 載入 → ⚡ 自動合併短趟 → 選蛙式 → 🏊 全部改此泳姿 → ⬇ 下載修正檔

實際範例（`23505296923_ACTIVITY.fit`，25 m 泳池）— **同一區段**合併前後條狀圖：

| 合併前 | 合併後（合併＋全改蛙式） |
|:---:|:---:|
| ![合併前條狀圖](docs/images/08-example-short-bars-before.png) | ![合併後條狀圖](docs/images/11-example-short-bars-after.png) |

| | 合併前 | 合併後 |
|---|--------|--------|
| 趟數 | 76 | **66** |
| 距離 | 1900 m | **1650 m** |
| 泳姿 | 蛙式／仰式混雜 | **全改蛙式**（藍色長條） |
| 操作 | — | ⚡ 自動合併 → 🏊 全部改此泳姿 |

更完整的合併前後對照、操作步驟與介面截圖見 **[docs/USAGE.md](docs/USAGE.md#自動合併功能--合併前後條狀圖)**。

## 使用方式

完整圖文說明請見 **[docs/USAGE.md](docs/USAGE.md)**（含自動合併前後截圖與 Release 安裝說明）。
